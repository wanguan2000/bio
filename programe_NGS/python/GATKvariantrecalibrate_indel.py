...
'''
class GATKVariantRecalibrate_indel:
    def __init__(self,config):
        self.const=config
    def GATKRecalibrate_indel(self,vcf_indel):
        ...
        cmd_VariantRecalibrator = ' '.join(['java','-Djava.io.tmpdir='+const.Temp_Folder,const.memory,'-jar',GATK,'-T VariantRecalibrator','-R',FastaDatabase,'-input',vcf_indel,'-resource:mills,known=true,training=true,truth=true,prior=12.0',const.indels_mills,'-an QD -an FS -an HaplotypeScore -an ReadPosRankSum ','--maxGaussians 4 --percentBadVariants 0.05','-mode INDEL','-recalFile',indel_recal,'-tranchesFile',indel_tranches,'-rscriptFile',indel_rscript])
