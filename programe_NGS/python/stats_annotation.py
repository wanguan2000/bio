...
class Stats_Anotation:
    ...
    def stats_snp(self, Input_cvs,key_stats=['Genotype','Gene_location','Exon_syno','dbSNP','1000G']):
        ...
        with open(Input_cvs, 'rU') as f:
            for line in f:
                if not line:
                    break
                if line.startswith('#CHROM'):
                    key_stats=['Genotype','Gene_location','Exon_syno','dbSNP','1000G']
    ...
    def stats_indel(self, Input_cvs,key_stats=['Genotype','Gene_location','Exon_syno','dbSNP','1000G']):
        ######data, software ,path  initialization#########
        outcvs = StringMeth().changeExtension(Input_cvs, '_stats.txt')
        het_hom={}
        Gene_location={}
        ...
        with open(Input_cvs, 'rU') as f:
            for line in f:
                if not line:
                    break
                if line.startswith('#CHROM'):
                    key_stats=['Variation_type','Genotype','Gene_location','Exon_syno','dbSNP','1000G','Variation_type']
                    sample_index = self.getindex(line,key_stats)
                else:
                   totalnumber = totalnumber +1
                   line = line.rstrip()
