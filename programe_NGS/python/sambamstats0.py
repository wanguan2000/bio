...
class SamBamStats:
    ...
    def SamStats(self,filename):
        ...
        result_list.append(str(total_read))
    ...
    def ReadsCover(self,filename, totalreads=44394028 * 2):
        ...
        with open(filename, 'rU') as f:
            ...
            for line in f:
                ...
                if float(spline[-4]) > 1:
                    sequenced_region += 1
            result_list.append(('Reads on target regions rate: %0.2f%%' % (target_reads * 100.0 / totalreads)))
            result_list.append(('Fraction of target covered by reads: %0.2f%%' % (sequenced_region * 100.0 / number_region)))
        return result_list
    ...
    def BaseCover(self,filename, total_Base=0):
        ...
        result_list.append(('total bases: %s' % (str(total_Base))))
        result_list.append(('Fraction of target bases covered: %0.2f%%' % ((1 - cover_list[0]) * 100)))
        result_list.append(('>=4X coverage: %0.2f%%' % ((1 - cover4) * 100)))
        result_list.append(('>=10X coverage: %0.2f%%' % ((1 - cover10) * 100)))
        result_list.append(('>=20X coverage: %0.2f%%' % ((1 - cover20) * 100)))
        result_list.append(('Mean coverage sequencing depth on target: %0.0f' % (map_Base / total_Base)))
        return result_list
