...
class GATKRealign:
    ...
    def GATKTargetCreatorIndelRealigner_SP(self,Input_Bam,FastaDatabase,dbsnp):
    ######data, software ,path  initialization#########
        const = self.const
        ...
        for bam in Input_Bam:
            #RealignerTargetCreator
            bam_intervals = StringMeth().changeExtension(bam,'.intervals')
            cmd_TargetCreator = ' '.join(['java',const.memory,'-jar',GATK,'-T RealignerTargetCreator','-I',bam,'-R',FastaDatabase,'-o',bam_intervals,'-known:',dbsnp])
            print cmd_TargetCreator
            os.system(cmd_TargetCreator)
            #IndelRealigner
            bam_realign = StringMeth().changeExtension(bam,'_realign.bam')
            cmd_realign = ' '.join(['java','-Djava.io.tmpdir='+const.Temp_Folder,const.memory,'-jar',GATK,'-T IndelRealigner','-I',bam,'-R',FastaDatabase,'-targetIntervals' ,bam_intervals,'-o',bam_realign,'-known:',dbsnp,'--consensusDeterminationModel KNOWNS_ONLY -LOD 0.4'])
            print cmd_realign
            os.system(cmd_realign)
            Output_List.append(bam_realign)
        return Output_List
