...
#print str(int(time.time()- 1337655100))
...
class bwaMethSP:
    def __init__(self,config):
        self.const=config
    def bwaPairendSP(self,Input_Pair_Files,AlignDatabase):
        ######data, software ,path  initialization#########
        const = self.const
        random_name = str(int(time.time()- 1337655100))
        ...
        for Pair_File in Input_Pair_Files:
            ...
            Output_Folder_Leftsai = Output_Folder + '/' + LeftName + random_name+ '.sai'
            Output_Folder_Rightsai = Output_Folder + '/' + RightName + random_name + '.sai'
            Output_Folder_sam = Output_Folder + '/' + LeftName + '_' + RightName +random_name + '.sam'
            LeftAlign = ' '.join([bwa, 'aln -t',const.cpu, '-l 80','-f', Output_Folder_Leftsai, AlignDatabase, leftFastq])
            print LeftAlign
            os.system(LeftAlign)

            RightAlign = ' '.join([bwa, 'aln -t',const.cpu, '-l 80','-f', Output_Folder_Rightsai, AlignDatabase, RightFastq])
            print RightAlign
    ...
    def bwaSinendSP(self,Input_Sin_Files,AlignDatabase):
        ...
        for Sin_Files in Input_Sin_Files:
            ...
            Output_Folder_sai = Output_Folder + '/' + NameFastq + random_name + '.sai'
            Output_Folder_sam = Output_Folder + '/' + NameFastq + random_name +'.sam'
            SinAlign = ' '.join([bwa, 'aln -t', const.cpu, '-l 80','-f', Output_Folder_sai, AlignDatabase, FastqFile])
            print SinAlign
            os.system(SinAlign)
    ...
    def bwaPairend(self,Input_Pair_Files,AlignDatabase):
        ######data, software ,path  initialization#########
        const = self.const
