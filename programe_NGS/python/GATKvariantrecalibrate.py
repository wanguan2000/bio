...
class GATKVariantRecalibrate:
    ...
    def GATKRecalibrate(self,Input_vcf):
        ...
        for bigvcf in Input_vcf:
            ...
            cmd_VariantRecalibrator = ' '.join(['java','-Djava.io.tmpdir='+const.Temp_Folder,const.memory,'-jar',GATK,'-T VariantRecalibrator','-R',FastaDatabase,'-input',vcf_snp,'-resource:hapmap,VCF,known=false,training=true,truth=true,prior=15.0',hapmap,'-resource:omni,VCF,known=false,training=true,truth=false,prior=12.0',omni,'-resource:dbsnp,VCF,known=true,training=false,truth=false,prior=6.0',dbsnp,'-an QD -an HaplotypeScore -an MQRankSum -an ReadPosRankSum -an FS -an MQ','--maxGaussians 6','-mode SNP','-recalFile',snp_recal,'-tranchesFile',snp_tranches,'-rscriptFile',snp_rscript])
            ...
            cmd_VariantRecalibrator = ' '.join(['java','-Djava.io.tmpdir='+const.Temp_Folder,const.memory,'-jar',GATK,'-T VariantRecalibrator','-R',FastaDatabase,'-input',vcf_indel,'-resource:mills,VCF,known=true,training=true,truth=true,prior=12.0',const.indels_mills,'-an QD -an FS -an HaplotypeScore -an ReadPosRankSum ','--maxGaussians 6','-mode INDEL','-recalFile',indel_recal,'-tranchesFile',indel_tranches,'-rscriptFile',indel_rscript])
            print cmd_VariantRecalibrator
            os.system(cmd_VariantRecalibrator)
