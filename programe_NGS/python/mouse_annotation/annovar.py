...
class Annovar:
    ...
    def geneAnnotation(self, filename, annovar, species='hg19', gene='refgene', database='humandb/', dnsnp='snp135',
                       ...
                       mirnatarget='mirnatarget', mce46way='mce46way',cosmic='cosmic.gff3',other_gff=''):
        #vcf2human
        outhuman = StringMeth().changeExtension(filename, '.'+species)
        ...
        ##other_gff ./annotate_variation.pl ST_Hqual_snp.human . -gff3dbfile cosmic.gff3 -dbtype gff3 -regionanno
        cmd = ' '.join([annovar + 'annotate_variation.pl', '--buildver',species,'-dbtype','gff3','-regionanno','-gff3dbfile',other_gff,outhuman,annovar+database])
    ...
    def csv_snp_indel(self,HqualVCF,meth='snpindel',species='mm9',dnsnp='snp132',ConsElements=''):
        if meth == 'snp':
            ##vcf
        ...
        ##gene
        gene = StringMeth().changeExtension(HqualVCF, '.'+species+'.variant_function')
        rootkey = [2, 3, 5]
        result = [0, 1]
        mydict = self.bedMarge(gene, rootkey, result, mydict)

        ##exon
        exon = StringMeth().changeExtension(HqualVCF, '.'+species+'.exonic_variant_function')
        rootkey = [3, 4, 6,]
        result = [1, 2]
        mydict = self.bedMarge(exon, rootkey, result, mydict)

        ###dbsnp135##
        dbsnp = StringMeth().changeExtension(HqualVCF, '.'+species+'.'+species+'_'+dnsnp+'_dropped')
        rootkey = [2, 3, 5, ]
        result = [1]
        ...
        '''
        ##mirna
        mirna = StringMeth().changeExtension(HqualVCF, '.'+species+'.'+species+'_wgRna')
        rootkey = [2, 3, 5,]
        result = [1]
        mydict = self.bedMarge(mirna, rootkey, result, mydict)

        ##mirnatarget
        mirnatarget = StringMeth().changeExtension(HqualVCF, '.'+species+'.'+species+'_targetScanS')
        rootkey = [2, 3, 5,]
        result = [1]
        mydict = self.bedMarge(mirnatarget, rootkey, result, mydict)

        ##mce46way
        mce46way = StringMeth().changeExtension(HqualVCF, '.'+species+'.'+species+'_phastConsElements'+ConsElements+'way')
        rootkey = [2, 3, 5,]
        result = [1]
        mydict = self.bedMarge(mce46way, rootkey, result, mydict)

        ##cosmic
        cosmic = StringMeth().changeExtension(HqualVCF, '.'+species+'.'+species+'_gff3')
        rootkey = [2, 3, 5,]
        result = [1]
    ...
    def do_pipeline(self, Input_vcf, meth='snp;indel'):
        ...
        if meth == 'snp':
            ...
            self.geneAnnotation(snp_Hqual, annovar,species='mm9', database='mousedb/',dnsnp='snp132',mce46way='mce30way',other_gff='mm9_TFBS.gff')
            #########result######
            [mydict,key_list,vcf_head] = self.csv_snp_indel(snp_Hqual,'snp',species='mm9',dnsnp='snp132',ConsElements='30')
            result_SNP = StringMeth().changeExtension(snp_Hqual, '.csv')

            vcf_head[1] = 'Start'
            vcf_head.insert(2,'End')

            head_snp = '\t'.join(vcf_head) + '\tALT_ratio\tVariation_type\tGenotype\tGene_location\tGene\tExon_syno\tExon\tdbSNP\tMirna\tMirnatarget\tConservedElements\tRegulation'
            ...
            self.geneAnnotation(indel_Hqual, annovar,species='mm9', database='mousedb/',dnsnp='snp132',mce46way='mce30way',other_gff='mm9_TFBS.gff')


            #########result######
            [mydict,key_list,vcf_head] = self.csv_snp_indel(indel_Hqual,'indel',species='mm9',dnsnp='snp132',ConsElements='30')
    ...
    def domuti_pipeline(self, Input_vcf, meth='snp;indel'):
    ######data, software ,path  initialization#########
        const = self.const
        #annovar = const.annovar
        annovar = '/gpfs/home/wanguan2000/NGSToolkit/software_NGS/annovar20120401/'
        ...
        if meth == 'snp':
            ...
            head_snp = '\t'.join(vcf_head) + '\tALT_ratio\tVariation_type\tGenotype\tGene_location\tGene\tExon_syno\tExon\tdbSNP\tMirna\tMirnatarget\tConservedElements\tRegulation'
            sphead_snp = head_snp.split('\t')

            Variation_type_index = sphead_snp.index('Variation_type')
            ...
            with open(result_SNP, mode='w') as f:
                ...
                for mykey in key_list:
                    spline = mydict[mykey].split('\t')
                    spline.insert(1,spline[1])
                    spline.insert(Variation_type_index,'SNP')
            ...
            sphead_snp = head_snp.split('\t')
            Variation_type_index = sphead_snp.index('Variation_type')
            REF_index = sphead_snp.index('REF')
            ...
            with open(result_indel, mode='w') as f:
                ...
                for mykey in key_list:
                    ...
                    if spline[REF_index].startswith('-'):
                        spline.insert(Variation_type_index,'insertion')
                    else:
                        spline.insert(Variation_type_index,'deletion')
