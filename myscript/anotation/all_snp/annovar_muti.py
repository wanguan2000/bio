...
class Annovar:
    ...
    def vcf_anotation(self, Input_vcf, meth='snp;indel'):
        ...
        if meth == 'snp':
            ...
            head_snp = '\t'.join(vcf_head) + '\tVariation_type\tGene_location\tGene\tExon_syno\tExon\tdbSNP\t1000G\tGWAS\tVisift\tTFbs\tMirna\tMirnatarget\tConservedElements\tcosmic'
    ...
    def seletVCF_snp(self,filename, rootkey=[0, 1, 3, 4]):
        mydict = {}
        key_list=[]
        result=[]
        vcf_head=[]
        ...
        with open(filename, 'rU') as f:
            for line in f:
                ...
                if not line.startswith('#'):
                    ...
                    mykey = '\t'.join([chr,str(stat),ref,alt])
                    key_list.append(mykey)
                    mydict[mykey] = '\t'.join([spline.__getitem__(elem) for elem in result])
...
snp_vcf= 'C3_ALLmerge_realign_mate_recal_raw_indel_filtered_somatic_all.vcf'
indel_vcf= 'C3_ALLmerge_realign_mate_recal_raw_indel_filtered_somatic_all.vcf'
