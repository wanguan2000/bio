...
class Stats_Anotation:
    ...
    def stats_snp(self, Input_cvs,key_stats=['Gene_location','Exon_syno','dbSNP','1000G']):
        ...
        with open(Input_cvs, 'rU') as f:
            for line in f:
                if not line:
                    break
                if line.startswith('#CHROM'):
                    key_stats=['Gene_location','Exon_syno','dbSNP','1000G']
                   ...
                   #13 1000G
                   if not spline[sample_index['1000G']].startswith('NA') and not spline[sample_index['1000G']].startswith('1000G') :
                       G1000 = G1000+1

        with open(outcvs, 'w') as infile:
            infile.write('Variant Statistics Items\t'+ re.sub('_.*', '_ALL', StringMeth().getPathName(Input_cvs)) +'\n')
            infile.write('Total SNPs number:\t'+ str(totalnumber) +'\n')
    ...
    def stats_indel(self, Input_cvs,key_stats=['Gene_location','Exon_syno','dbSNP','1000G']):
        ######data, software ,path  initialization#########
        outcvs = StringMeth().changeExtension(Input_cvs, '_stats.txt')
        het_hom={}
        Gene_location={}
        ...
        with open(Input_cvs, 'rU') as f:
            for line in f:
                if not line:
                    break
                if line.startswith('#CHROM'):
                    key_stats=['Gene_location','Exon_syno','dbSNP','1000G','Variation_type']
                    sample_index = self.getindex(line,key_stats)
                else:
                   totalnumber = totalnumber +1
                   line = line.rstrip()
        ...
        with open(outcvs, 'w') as infile:
            infile.write('Variant Statistics Items\t'+ re.sub('_.*', '_ALL', StringMeth().getPathName(Input_cvs)) +'\n')
            infile.write('Total Indels number:\t'+ str(totalnumber) +'\n')
