...
class Stats_Exon_Position:
    ...
    def writeresult(self, resultinfo, redict, collist, myresult, name='snp'):
        result2 = []
        ...
        with open('position_' + name + '_allsamples.xls', 'w') as infile:
            infile.write('#Frequency\t' + '\t'.join(resultinfo) + '\t' + '\t'.join(collist) + '\n')
            infile.write('\n'.join(result2))
    ...
    def pipeline(self,):
        ...
        for dirname in os.listdir('.'):
            if '_somatic_snp_Hqual.xls' in dirname and not dirname.startswith('.'):
                filelist.append(dirname)
        redict, collist, myresult = self.getSNP(filelist, keyroot, resultinfo, QUAL='QUAL')
        self.writeresult(resultinfo, redict, collist, myresult, 'SNP')
        #indel
        filelist = []
        for dirname in os.listdir('.'):
            if '_somatic_indel_Hqual.xls' in dirname and not dirname.startswith('.'):
                filelist.append(dirname)
        redict, collist, myresult = self.getSNP(filelist, keyroot, resultinfo, QUAL='QUAL')
        self.writeresult(resultinfo, redict, collist, myresult, 'indel')
