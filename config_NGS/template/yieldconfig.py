...
#pipeline commad
    with open(Outfold+'/command.txt','a') as cmd:
        if not n[0]%4:
           cmd.write('cd /gpfs/home/wanguan2000/NGSToolkit/\n')
        cmd.write(("nohup python pipeline_NGS/"+pipeline+" config_NGS/"+Outfold+"/config_%s.py >nohup_NGS/"+Outfold+"/nohup_%s.txt 2>&1 &\n") % (dir,dir))
    n[0]=n[0]+1
